import argparse
import logging

# locally defined agent
from agent import DemoAgentArgs

# browsergym experiments utils
from browsergym.experiments import EnvArgs, ExpArgs, get_exp_result

import tasks

def str2bool(v):
    if isinstance(v, bool):
        return v
    if v.lower() in ("yes", "true", "t", "y", "1"):
        return True
    elif v.lower() in ("no", "false", "f", "n", "0"):
        return False
    else:
        raise argparse.ArgumentTypeError("Boolean value expected.")


def parse_args():
    parser = argparse.ArgumentParser(description="Run experiment with hyperparameters.")
    parser.add_argument(
        "--model_name",
        type=str,
        default="gpt-4o",
        help="OpenAI model name.",
    )
    parser.add_argument(
        "--vlm_name",
        type=str,
        default=None,
        help="(option) VLM name.",
    )
    parser.add_argument(
        "--task_name",
        type=str,
        default="openended",
        help="Name of the Browsergym task to run. You can't choose 'openended' here.",
    )
    parser.add_argument(
        "--use_html",
        type=str2bool,
        default=False,
        help="Use HTML in the agent's observation space.",
    )
    parser.add_argument(
        "--use_axtree",
        type=str2bool,
        default=True,
        help="Use AXTree in the agent's observation space.",
    )
    parser.add_argument(
        "--result_dir",
        type=str,
        default="./results",
        help="Directory to save the experiment results.",
    )

    return parser.parse_args()


def main():
    print(
        """\
--- WARNING ---
This is a basic agent for demo purposes.
"""
    )

    args = parse_args()

    # To use other agent, change here
    agent_args = DemoAgentArgs(
        model_name=args.model_name,
        vlm_name=args.vlm_name,
        chat_mode=False,
        demo_mode="off",
        use_html=args.use_html,
        use_axtree=args.use_axtree,
        use_screenshot=False
    )

    # setting up environment config
    env_args = EnvArgs(
        task_name=args.task_name,
        task_seed=None,
        max_steps=30,
        headless=True,  
    )

    # for openended task, set environment and agent to interactive chat mode on a start url
    if args.task_name == "openended":
        quit("Openended task is not supported in this script.")
        
    # setting up the experiment
    exp_args = ExpArgs(
        env_args=env_args,
        agent_args=agent_args,
    )

    #exp_args.logging_level = logging.DEBUG
    #exp_args.logging_level_stdout = logging.DEBUG

    # running and logging results
    exp_args.prepare(args.result_dir)
    exp_args.run()

    # loading and printing results
    exp_result = get_exp_result(exp_args.exp_dir)
    exp_record = exp_result.get_exp_record()

    for key, val in exp_record.items():
        print(f"{key}: {val}")


if __name__ == "__main__":
    main()
